...
| `Features/Stories/StoryValidation.cs` | Story, chapter, scene, arc and beat input checks. Structural only; nothing compares a scene or a beat with the lore or with another. |
...
| `src/stories/` | Story, chapter, scene and plot API client, DTO types, the scene date, count, chapter and arc labels (`format.ts`), and a container's scenes, the story's reading order and which beats point at each scene (`structure.ts`). |
...
| `Lorex.Api.Tests/StoryBackupTests.cs` | Stories in a version 7 backup: order, chapters and each scene's place in one, references, no copied lore, determinism, and version 5 and 4 files. |
...
| `Lorex.Api.Tests/PlotBackupTests.cs` | Plot in a version 7 backup: order, text, sorted link ids, nothing derived, references resolving, determinism, and a version 6 file. |
