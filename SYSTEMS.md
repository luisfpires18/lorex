...
| Lore address | Where the Lore browser is lives in its URL and nowhere else: `lore?type=<entity type id>&status=<0\|1\|2>&q=<text>&page=<n>`. The type is an id, never a name; one the universe does not have reads as All and the address is corrected in place. Choosing a type or a page pushes a history entry; typing and the status replace the one they are on. A new entry opened at `lore/new?type=<id>` starts in that type if the universe has it, and stays changeable. |
...
| `Lorex.E2E/specs/type-filter.spec.ts` | Type icons chosen and removed on the Types screen; Lore's types as addresses - chosen, reloaded, walked back and forward, kept by an opened entry, its type crumb and a new entry's starting type - custom types alike, an unknown id falling back to All; the filter and status within a type, the two empty states, the Trash and the keyboard; one scrolling row on a desktop and the Type menu, Filters and bottom create on a phone; the Lore browser filling a desktop column. |
